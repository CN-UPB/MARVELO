...
CoherenceDriftEstimator_Block = CoherenceDriftEstimator(cw_params,args.Name)
...
while 1:
    ...
    coherence1 = np.fromfile(inputs[0], dtype=np.complex128, count=args.FFTSize)
    coherence2 = np.fromfile(inputs[1], dtype=np.complex128, count=args.FFTSize)
    ...
    for pipe in outputs:
        pipe.write(coherenceDrift['value'])
        pipe.flush()
