...
frame1 = np.fromfile(input1, dtype='i2', count=PACKETSIZE/2) #item size is 2 bytes!
...
while packet.size == PACKETSIZE/2:
    ...
    #print/write output
    print(sro_info['value2'])
    #sys.stdout.flush()
    #for f in logfiles:
    #    f.write(str(sro_info['value']) + "\n")

    ##read next packet from input
    packet = np.fromfile(input1, dtype='i2', count=PACKETSIZE/2)
