...
#open input pipe
rate, input = scipy.io.wavfile.read('/home/hafifi/Desktop/docPaderborn/asnRepo/p1/Testbench/asn_server_Emulator/asn_server/Testbench/system/readfile_module/AudioData/Audio_2Chan_80ppm.wav')
...
packet = input[0:1024, :] # np.fromfile(input, dtype='i2', count=2048) #item size is 2 bytes!
...
while packet.size == PACKETSIZE/2:
    ...
    DataBlock = packet # np.reshape(packet, (1024, 2))

    # Resampling of streams
    DataBlock1 = Resampling_Block1.process_data(DataBlock[:,1])
    ...
    #packet = np.fromfile(input, dtype='i2', count=PACKETSIZE/2)
    packet = input[0+packetCount*1024:1024+packetCount*1024, :]
    packetCount += 1
    blockCount += 1
