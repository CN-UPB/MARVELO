...
PACKETSIZE = 2048 #receive data in 4KB packets (2KB or 1024 samples per channel equals shift amount used in processing blocks!)
...
while packet.size == PACKETSIZE/2:
    ...
    DataBlock1 = DataBlock
    #DataBlock2 = DataBlock[:, 0]

    #write channels to seperate outputs
    for o in outputs:
        o.write(DataBlock1.copy(order = 'C')) #must be written as c contiguous bytestream (left/right channel are interleaved)
        o.flush()
        #print 'buffering a packet'
        #sys.stdout.flush()

    #outputs[1].write(DataBlock2.copy(order = 'C'))
    #outputs[1].flush()
