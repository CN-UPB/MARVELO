...
class read_data(PythonJob):
	...
	def __init__(self, samples_per_packet = [2048], outputs=None):
	...
	def run(self):
		...
		_, data = wavfile.read('/home/konrad/marvelo-demo/examples/DXCP_logging/signal_file.wav')
		...
		while True:
			for num_samples in self.samples_per_packet:
				time_in = time.time_ns()
				sizeLoop = int(nr_samples/num_samples)
			
				for counter in range(sizeLoop):
					outputData = x[np.arange(counter*num_samples, (counter+1)*num_samples), :]
					outputData_0 = outputData[:, 0].copy(order='C')
					outputData_1 = outputData[:, 1].copy(order='C')
					#print(f'read_data {counter}/{sizeLoop}')
				
					log_arr = ["read_data", self.p.pid, 0, self.p.cpu_percent()/psutil.cpu_count(), self.p.memory_percent(), time_in, time.time_ns()]
				
					yield  (outputData_0, outputData_1, log_arr)
					
				print(f'read_data done with num_samples={num_samples}')
				time.sleep(30)
...
class dxcp_phat(PythonJob):
	...
	def run(self, input0, input1):
		...
		outputData = np.array([SROppm_est_out, TimeOffsetEndSeg_est_out, num_samples], dtype=np.float32)
		...
		return (outputData[0], outputData[1], outputData[2], log_arr)


class print_results(PythonJob, LocalJob):
	...
	def run(self, input0, input1, input2):
		time_in = time.time_ns()
		...
		if self.pktCntr >= ResetPeriod_NumFrInp:
			print(f'Past-Seg: SRO_ppm = {input0}; TimeOffset_smp = {input1}; num_samples = {input2}')
			self.text_file.write(f'Past-Seg: SRO_ppm = {input0}; TimeOffset_smp = {input1}; num_samples = {input2}')
			self.pktCntr = 0
		else:
			self.pktCntr += 1
