...
while True:
    ...
    inputData_0 = os.read(int(args.inputs[0]), FrameSize_input*4)
    inputData_1 = os.read(int(args.inputs[1]), FrameSize_input*4)
    ...
    SROppm_est_out = OutputDXCPPhaT['SROppm_est_out']
    TimeOffsetEndSeg_est_out = OutputDXCPPhaT['TimeOffsetEndSeg_est_out']

    # write output data into pipes
    #outputData = np.array([SROppm_est_out, TimeOffsetEndSeg_est_out], dtype=np.float64)
