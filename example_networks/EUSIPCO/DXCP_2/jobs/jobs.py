...
class read_data(PythonJob):
	...
	def __init__(self, channel=0, outputs=None):
	...
	def run(self):
		...
		sizeLoop = int(nr_samples/PACKETSIZE)
		
		while True:
			time_in = time.time_ns()
			for counter in range(sizeLoop):
				outputData = x[np.arange(counter*PACKETSIZE, (counter+1)*PACKETSIZE), :]
				...
				if self.channel == 0:
					yield  (outputData_0, log_arr)
				else:
					yield  (outputData_1, log_arr)
			print(f'read_data done')
			time.sleep(30)
