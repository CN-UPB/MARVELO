...
from fission/core/jobs/jobs.py import LocalJob
...
class cov1svd(PythonJob):
	...
	def run(self, audio1, audio2):
		if self.state == 0 and type(audio1) != int and type(audio2) != int:
			...
			return ((NO_ACTION, NO_ACTION, self.x[begin:end]))
...
class whiten(PythonJob):
	...
	def run(self, input1, input2, input3):
		if self.state == 0:
			...
			return (self.xn[begin:end],self.xn[begin:end])
...
class normfn(PythonJob):
	...
	def run(self, input1):
		if self.state == 0 and type(input1) != int:
			...
			return tuple([self.norm[begin:end]])
...
class cov2svd(PythonJob,LocalJob):
	def setup(self, *args, **kwargs):
		super().setup(*args, **kwargs)
		self.norm = np.zeros(0)
		self.xn = np.zeros(0)
