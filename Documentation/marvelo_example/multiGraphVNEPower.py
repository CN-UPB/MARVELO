...
arcsfile = "substrate_links_fynn_example.csv"# 'a' + str(nr_nodes) +str(seed)+ '.csv'
...
model.powertx = Var(model.nodes* model.timeslots, domain=NonNegativeReals, bounds=(0, 1))
...
# A node may send the traffic to itself and then to the others without actually running the block or receiving it from other node
def preventSelfLoop(model,component,port,node):
    preds_p = filter(lambda x: (x[2] == component and x[1] == port), model.vnf)
    incoming = filter(lambda x: x[1] == node, model.edges)
    succ_p = filter(lambda x: (x[0] == component and x[1] == port), model.vnf)

    if len(preds_p)>0 :
        # return model.h[component,port,node,node]*len(preds_p) <= sum(model.h[component,port,e] for e in incoming)*len(preds_p)+ model.map[component,port,node]*len(succ_p)#sum(model.h[c[0],c[1],e] for e in incoming for c in preds_p)
...
def SINR(model, source, destination, timeslot):
    ...
    interference =sum(
        model.powertx[v, timeslot] * arc_data.ix[(v, destination), 'Attenuation'] for
...
try:
    ...
    #          'f':[i for i in model.fwdNode if model.fwdNode[i].value > 0.9], 'map':[i for i in model.map if model.map[i].value >0.9]}, 'Power' + str(len(node_set)) +'Topology'+str(topology) + 'Napps'+ str(nr_apps)+'Cap'+str(nodeCapacity)+ str(trialx)+ '.pickle')
